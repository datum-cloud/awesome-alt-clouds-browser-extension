...
function makeCriterionRow(icon, labelText, cls, titleText) {
  const row = document.createElement('div');
  row.className = 'criterion-row';

  const iconEl = document.createElement('span');
  iconEl.className = 'criterion-icon';
  iconEl.textContent = icon;

  const labelEl = document.createElement('span');
  labelEl.className = cls ? `criterion-label ${cls}` : 'criterion-label';
  labelEl.textContent = labelText;
  if (titleText) labelEl.title = titleText;

  row.append(iconEl, labelEl);
  return row;
}

function makeCriteriaSummary(text) {
...
function makeMetaRow(label, value, titleAttr) {
  const row = document.createElement('div');
  ...
  if (titleAttr) valueEl.title = titleAttr;
...
function buildCriteriaBlock(criteria, score) {
  const statusIcon = { met: '✅', unmet: '❌', unknown: '❔' };
  const statusClass = { met: '', unmet: 'missing', unknown: 'unknown' };
  ...
  if (allUnknown && score === 2) {
    block.appendChild(makeCriterionRow(
      '❔',
      'Meets 2 of 3 criteria — specific gap not tracked in current data',
      'unknown',
    ));
    for (const c of CRITERIA_META) {
      block.appendChild(makeCriterionRow('❔', c.label, 'unknown'));
    }
    block.appendChild(makeCriteriaSummary('Per-criterion breakdown will improve in a future data update.'));
    return block;
  }

  for (const c of CRITERIA_META) {
    const status = criteria[c.key] ?? 'unknown';
    block.appendChild(makeCriterionRow(statusIcon[status], c.label, statusClass[status], c.desc));
  }

  if (unmetKeys.length > 0) {
    const missing = unmetKeys
      .map(k => CRITERIA_META.find(c => c.key === k)?.label)
      .filter(Boolean);
    block.appendChild(makeCriteriaSummary(`Missing: ${missing.join(', ')}`));
  } else if (allUnknown) {
    block.appendChild(makeCriteriaSummary(`Meets ${score ?? '?'} of 3 — per-criterion detail unavailable`));
  }
...
function renderListed(state, entry, tab) {
  ...
  document.getElementById(`${prefix}-criteria`)
    .replaceChildren(buildCriteriaBlock(entry.criteria, entry.score));
...
function renderWatch(entry, tab) {
  ...
  document.getElementById('watch-criteria')
    .replaceChildren(buildCriteriaBlock(entry.criteria, entry.score));
...
async function renderPlus(tab) {
  ...
  metaEl.replaceChildren(
    makeMetaRow('Domain', meta.hostname),
    makeMetaRow('Title', meta.title, meta.title),
  );
